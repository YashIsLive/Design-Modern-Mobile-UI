...
import {
  ...
  ChevronRight, Calendar, Weight, Utensils, Activity, Bell, Search, X, LogOut
} from "lucide-react";

type Tab = "children" | "mothers";
type FilterPriority = "all" | "urgent" | "monitor" | "healthy";
...
export function DashboardScreen({ onAddBeneficiary, onAITutor, onLogout }: { onAddBeneficiary: () => void; onAITutor: () => void; onLogout: () => void }) {
  const [tab, setTab] = useState<Tab>("children");
  const [filterPriority, setFilterPriority] = useState<FilterPriority>("all");
  const [searchQuery, setSearchQuery] = useState("");
  const [showNotifications, setShowNotifications] = useState(false);
  
  const allCards = tab === "children" ? childCards : motherCards;

  // Filter by priority
  const priorityFilteredCards = useMemo(() => {
    if (filterPriority === "all") return allCards;
    return allCards.filter(card => card.priority === filterPriority);
  }, [allCards, filterPriority]);

  // Filter by search query
  const filteredCards = useMemo(() => {
    if (!searchQuery.trim()) return priorityFilteredCards;
    const query = searchQuery.toLowerCase();
    return priorityFilteredCards.filter(card =>
      card.name.toLowerCase().includes(query) ||
      card.nameHi.toLowerCase().includes(query) ||
      card.tag.toLowerCase().includes(query) ||
      card.age.toLowerCase().includes(query)
    );
  }, [priorityFilteredCards, searchQuery]);

  const notifications = [
    { id: 1, type: "urgent", message: "Aarav Sharma requires immediate attention", messageHi: "आरव शर्मा को तत्काल ध्यान देने की आवश्यकता है" },
    { id: 2, type: "monitor", message: "Priya missed scheduled checkup", messageHi: "प्रिया ने निर्धारित जांच को मिस किया" },
    { id: 3, type: "healthy", message: "Rahul's latest checkup is clear", messageHi: "राहुल की नवीनतम जांच स्पष्ट है" },
  ];

  return (
    <div className="flex flex-col h-full relative" style={{ background: "#FDF6EE" }}>
      {/* Sub-header: Search + notifications + logout (FIXED) */}
      <div className="hidden" style={{ background: "#1A2E4A" }}>
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <input
              ...
              placeholder="Search beneficiaries / लाभार्थी खोजें"
              className="flex-1 bg-transparent outline-none"
              style={{ fontSize: "0.72rem", color: "rgba(255,255,255,0.9)" }}
            />
            {searchQuery && (
      ...
      <AnimatePresence>
        {showNotifications && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-14 right-4 z-50 rounded-2xl shadow-lg"
            style={{ background: "#fff", width: 280, maxHeight: 300, overflowY: "auto" }}
          >
            <div className="p-3 border-b" style={{ borderColor: "rgba(26,46,74,0.1)" }}>
              <p className="font-bold" style={{ fontSize: "0.82rem", color: "#1A2E4A" }}>Notifications</p>
            </div>
            {notifications.map(notif => {
              const cfg = PRIORITY_CONFIG[notif.type as "urgent" | "monitor" | "healthy"];
              return (
                <div key={notif.id} className="p-3 border-b last:border-b-0" style={{ borderColor: "rgba(26,46,74,0.08)" }}>
                  <div className="flex gap-2">
                    ...
                    <div className="flex-1">
                      <p style={{ fontSize: "0.7rem", color: "#1A2E4A", fontWeight: 600, lineHeight: 1.4 }}>
                        {notif.message}
                      </p>
                      <p style={{ fontSize: "0.6rem", color: "#6B7A8D", fontFamily: "'Noto Sans Devanagari', sans-serif", marginTop: 2 }}>
                        {notif.messageHi}
                      </p>
                    </div>
                  </div>
      ...
      <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
        ...
        {/* Segment control */}
        <div className="px-4 py-3 flex-shrink-0" style={{ background: "#1A2E4A" }}>
          <div
        ...
        <div className="px-4 py-2 flex gap-1.5 overflow-x-auto flex-shrink-0" style={{ background: "#1A2E4A", scrollbarWidth: "none" }}>
          {[
            { id: "all" as FilterPriority, label: "All / सभी", color: "#fff" },
            { id: "urgent" as FilterPriority, label: "Urgent / अत्यावश्यक", color: "#EF4444" },
            { id: "monitor" as FilterPriority, label: "Monitor / निगरानी", color: "#F59E0B" },
            { id: "healthy" as FilterPriority, label: "Healthy / स्वस्थ", color: "#10B981" },
          ].map(filter => (
            <motion.button
              key={filter.id}
              whileTap={{ scale: 0.95 }}
              onClick={() => setFilterPriority(filter.id)}
              className="flex-shrink-0 px-3 py-1.5 rounded-full font-semibold transition-all"
              style={{
                background: filterPriority === filter.id ? "rgba(255,255,255,0.2)" : "rgba(255,255,255,0.08)",
                border: `1.5px solid ${filterPriority === filter.id ? filter.color + "60" : "rgba(255,255,255,0.1)"}`,
                color: filter.color,
                fontSize: "0.68rem",
              }}
            >
        ...
        {/* Stats strip */}
        <div className="px-4 py-2.5" style={{ background: "#1A2E4A" }}>
          <div className="flex gap-2">
            {[
              { label: "Total", labelHi: "कुल", val: allCards.length.toString(), color: "#fff" },
              { label: "Urgent", labelHi: "अत्यावश्यक", val: allCards.filter(c => c.priority === "urgent").length.toString(), color: "#EF4444" },
              { label: "Monitor", labelHi: "निगरानी", val: allCards.filter(c => c.priority === "monitor").length.toString(), color: "#F59E0B" },
              { label: "Healthy", labelHi: "स्वस्थ", val: allCards.filter(c => c.priority === "healthy").length.toString(), color: "#10B981" },
            ].map((s, i) => (
              <div
                key={i}
        ...
        {filteredCards.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-40">
            <p style={{ fontSize: "0.88rem", color: "#6B7A8D", textAlign: "center" }}>
              {searchQuery
                ? "No beneficiaries found / कोई लाभार्थी नहीं मिला"
                : "No beneficiaries in this category / इस श्रेणी में कोई लाभार्थी नहीं"}
            </p>
          </div>
        ) : (
          <>
            <div className="flex items-center justify-between mb-3">
              <p className="font-semibold" style={{ fontSize: "0.82rem", color: "#1A2E4A" }}>
                {searchQuery ? "Search Results" : "Priority List"}{" "}
                <span style={{ fontFamily: "'Noto Sans Devanagari', sans-serif", fontWeight: 400, color: "#6B7A8D", fontSize: "0.72rem" }}>
                  {searchQuery ? "/ खोज परिणाम" : "/ प्राथमिकता सूची"} ({filteredCards.length})
                </span>
              </p>
            </div>
            ...
            <AnimatePresence mode="wait">
              <motion.div
                key={`${tab}-${filterPriority}-${searchQuery}`}
